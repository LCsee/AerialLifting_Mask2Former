...
class MaskFormer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.zyq_code = zyq_code
        self.zyq_mapping = zyq_mapping
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            'zyq_code': cfg.zyq_code,
            'zyq_mapping': cfg.zyq_mapping
        }
    ...
    def panoptic_inference_scannet_reduced(self, mask_cls, mask_pred):
        if self.zyq_mapping:
            scannet_thing_ids, num_scannet_classes = get_thing_semantics_zyq()
            coco_to_scannet, invalid_classes = get_ade_to_custom_zyq()
        ...
        if cur_masks.shape[0] == 0:
            ...
            return panoptic_seg, segments_info, pano_seg_probabilities, pano_seg_mask_preds

    def panoptic_inference_scannet_reduced_separate_masks(self, mask_cls, mask_pred):
        if self.zyq_mapping:
            scannet_thing_ids, num_scannet_classes = get_thing_semantics_zyq()
            coco_to_scannet, invalid_classes = get_ade_to_custom_zyq()
        else:
        ...
        # NOTE 这个目的就是为了下面的scores和labels的计算，直接用下面的原始函数<panoptic_inference_coco>得到这两个值
        smax_reduced_cur_mask_cls_scannet = torch.zeros([smax_cur_mask_cls_scannet.shape[0], num_scannet_classes], device=mask_cls.device)
        torch_scatter.scatter_add(smax_cur_mask_cls_scannet, index=torch.tensor(coco_to_scannet, device=mask_cls.device).long(), out=smax_reduced_cur_mask_cls_scannet)
        scores, labels = smax_reduced_cur_mask_cls_scannet.max(-1)
        ...
        # scores, labels = smax_cur_mask_cls_scannet.max(-1)
        # keep = torch.ones_like(scores > self.object_mask_threshold).bool()
        # cur_masks = pre_cur_masks[keep]
        # cur_mask_cls_probabilities = smax_cur_mask_cls_scannet[keep]

        return cur_mask_cls_probabilities, cur_masks

    def post_tta_merging(self, cur_mask_cls_probabilities, cur_masks):
        ...
        if self.zyq_code:
            if self.zyq_mapping:
                ...
                scannet_thing_ids = list(self.metadata.thing_dataset_id_to_contiguous_id.values())
                num_scannet_classes = len(self.metadata.stuff_dataset_id_to_contiguous_id)
        else:
            scannet_thing_ids, num_scannet_classes = get_thing_semantics()
        processed_results = [{}]
    ...
    def panoptic_inference(self, mask_cls, mask_pred):
        if self.zyq_code:
            if self.zyq_mapping:
                return self.panoptic_inference_scannet_reduced_separate_masks(mask_cls, mask_pred)
            else:
                return self.panoptic_inference_coco_zyq(mask_cls, mask_pred)
        else:
            return self.panoptic_inference_scannet_reduced_separate_masks(mask_cls, mask_pred)
    

    # NOTE 这个函数是原始mask2former的panoptic_inference，panolift 作者改成上面的 panoptic_inference
    ...
    def panoptic_inference_coco_zyq(self, mask_cls, mask_pred):
        scores, labels = F.softmax(mask_cls, dim=-1).max(-1)
        mask_pred = mask_pred.sigmoid()
