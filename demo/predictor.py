...
from detectron2.utils.visualizer import ColorMode as ColorMode_zyq
from detectron2.utils.visualizer import Visualizer as Visualizer_zyq
...
class VisualizationDemo(object):
    def __init__(self, cfg, instance_mode=ColorMode.IMAGE, parallel=False):
        ...
        self.zyq_code = cfg.zyq_code
        self.zyq_mapping = cfg.zyq_mapping
        if self.zyq_code:
            self.instance_mode = ColorMode_zyq.IMAGE
        else:
    ...
    def run_on_image(self, image, visualize=True):
        ...
        if visualize:
            if self.zyq_code:
                if self.zyq_mapping:
                    vis_output = self.visualize_predicions_zyq_mapping(image, predictions)
                else:
                    vis_output = self.visualize_predicions_zyq_original(image, predictions)
            else:
    ...
    def visualize_predicions_zyq_mapping(self, image, predictions):
        ...
        self.metadata.stuff_colors = self.colors
        self.metadata.thing_colors = self.colors
        self.metadata.stuff_classes = get_scannet_classes_zyq_mapping()
        self.metadata.thing_classes = get_scannet_classes_zyq_mapping()
        visualizer = Visualizer(image, self.metadata, instance_mode=self.instance_mode)
        vis_output = None
        if "panoptic_seg" in predictions:
            panoptic_seg, segments_info, panoptic_class_probs, panoptic_mask_conf = predictions["panoptic_seg"]
    ...
    def visualize_predicions_zyq_original(self, image, predictions):
        # Convert image from OpenCV BGR format to Matplotlib RGB format.
        image = image[:, :, ::-1]
        # self.metadata = EasyDict()
        # self.metadata.stuff_colors = self.colors
...
class TTAPredictor:
    ...
    def __call__(self, original_image):
        ...
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            ...
            return predictions
    # NOTE run_post_augmentation 调用这个
